...
function togglePaymentMethod() {
  if (pagamentoBonifico && pagamentoBonifico.checked) {
    ...
    if (emailPagamentoInput) emailPagamentoInput.value = '';
...
if (pagamentoBonifico) pagamentoBonifico.addEventListener('change', togglePaymentMethod);
if (pagamentoCarta) pagamentoCarta.addEventListener('change', togglePaymentMethod);
...
function submitContattiForm(event) {
  ...
  const body = encodeURIComponent(`Problema segnalato:\n${problema}\n\nEmail: ${email}\nTelefono: ${telefono || 'Non specificato'}`);
...
document.addEventListener('click', function(event) {
  ...
  if (event.target === modal) closeContattiModal();
});
document.addEventListener('keydown', function(event) {
  if (event.key === 'Escape') closeContattiModal();
});
...
function initializeFirstSignaturePad() {
  ...
  if (legaleCanvas) legaleSignaturePad = new SignaturePad(legaleCanvas);
...
function setupSignatureTypeToggle(partecipanteNum) {
  ...
  if (section) {
    const textInput = section.querySelector('.signature-text-input');
    if (textInput) textInput.addEventListener('input', function() { updateTextSignaturePreview(this); });
  }
}
...
function setupPrivacySignatureTypeToggle() {
  ...
  if (textInput) textInput.addEventListener('input', updatePrivacyTextSignaturePreview);
}
...
function setupLegaleSignatureTypeToggle() {
  ...
  if (textInput) textInput.addEventListener('input', updateLegaleTextSignaturePreview);
}
...
function clearLegaleSignature() {
  if (legaleSignaturePad) legaleSignaturePad.clear();
  const textInput = document.getElementById('legale-signature-text');
  if (textInput) {
    textInput.value = '';
    updateLegaleTextSignaturePreview();
...
function resetToSinglePartecipante() {
  ...
  if (firstTitle) firstTitle.textContent = 'Dati Personali';
...
function updatePartecipantiTitles() {
  ...
  sections.forEach((section, index) => {
    ...
    if (title) title.textContent = `Partecipante ${index + 1}`;
...
function updateRemoveButtons() {
  ...
  sections.forEach((section, index) => {
    ...
    if (removeBtn) removeBtn.style.display = (isAzienda && sections.length > 1) ? 'block' : 'none';
  });
}
...
function addPartecipante() {
  ...
  setTimeout(() => newSection.classList.remove('new-partecipante'), 300);
}
...
function generateCertifiedSignatureImage() {
  ...
  if (nome) ctx.fillText(`Firmatario: ${nome}`, 20, 65);
  if (cf) ctx.fillText(`C.F.: ${cf}`, 20, 85);
  if (provider) ctx.fillText(`Provider: ${provider}`, 20, 105);
...
function generateExcelFile(partecipanti, aziendaData, corso, prezzo, dataPrivacy, metodoPagamento, emailPagamento) {
  ...
  partecipanti.forEach((p, index) => {
    ...
    if (metodoPagamento === 'carta') row['Email Link Pagamento'] = emailPagamento || '';
...
iscrizioneForm.addEventListener("submit", async function(e) {
  ...
  try {
    ...
    if (isAzienda && aziendaData) {
      ...
      yPosition += 8;
      if (aziendaData.pec) { doc.text(`PEC: ${aziendaData.pec}`, 20, yPosition); yPosition += 8; }
      if (aziendaData.telefono) { doc.text(`Telefono: ${aziendaData.telefono}`, 20, yPosition); yPosition += 8; }
      if (aziendaData.settore) { doc.text(`Settore di Appartenenza: ${aziendaData.settore}`, 20, yPosition); yPosition += 8; }
      yPosition += 10;
    ...
    partecipanti.forEach((p, index) => {
      ...
      doc.setFontSize(12);
      doc.text(`Nome: ${p.nome}`, 20, yPosition); yPosition += 8;
      doc.text(`Cognome: ${p.cognome}`, 20, yPosition); yPosition += 8;
      doc.text(`Codice Fiscale: ${p.codiceFiscale}`, 20, yPosition); yPosition += 8;
      doc.text(`Email: ${p.email}`, 20, yPosition); yPosition += 8;
      if (!isAzienda && p.telefono) { doc.text(`Telefono: ${p.telefono}`, 20, yPosition); yPosition += 8; }
      doc.text(`Profilo Professionale: ${p.profilo}`, 20, yPosition); yPosition += 8;
      if (!isAzienda && p.settore) { doc.text(`Settore: ${p.settore}`, 20, yPosition); yPosition += 8; }
      yPosition += 2;

      if (!isAzienda) {
        doc.text(`Titolo Corso: ${corso}`, 20, yPosition); yPosition += 8;
        doc.text(`Quota di partecipazione: EUR ${prezzo}`, 20, yPosition); yPosition += 15;

        if (metodoPagamento === 'bonifico') {
          doc.text(`Metodo di pagamento: Bonifico Bancario`, 20, yPosition); yPosition += 8;
          doc.text(`Intestatario: MICHELA COCCO`, 20, yPosition); yPosition += 8;
          doc.text(`Banca: BANCA FINDOMESTIC`, 20, yPosition); yPosition += 8;
          doc.text(`IBAN: IT65U0311514000000000005566`, 20, yPosition); yPosition += 15;
        } else if (metodoPagamento === 'carta' && emailPagamento) {
          doc.text(`Metodo di pagamento: Carta di Credito/Debito/Google Pay`, 20, yPosition); yPosition += 8;
          doc.text(`Email per link pagamento: ${emailPagamento}`, 20, yPosition); yPosition += 15;
        }

        doc.text(`Firma del Partecipante:`, 20, yPosition);
        if (p.firma) doc.addImage(p.firma, 'PNG', 20, yPosition + 5, 100, 50);
    ...
    if (isAzienda) {
      ...
      if (aziendaData.firmaLegale) doc.addImage(aziendaData.firmaLegale, 'PNG', 20, yPosition, 100, 50);
      ...
      if (aziendaData.tipoFirmaLegale === 'certified') {
        ...
        if (aziendaData.firmaCertificata) {
          doc.text(`Firmatario: ${aziendaData.firmaCertificata.nome}`, 20, yPosition); yPosition += 6;
          doc.text(`Codice Fiscale: ${aziendaData.firmaCertificata.cf}`, 20, yPosition); yPosition += 6;
    ...
    const now = new Date();
    const timestamp = `${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}_${String(now.getHours()).padStart(2, '0')}${String(now.getMinutes()).padStart(2, '0')}`;
    lastGeneratedFileName = isAzienda ?
    ...
    const response = await fetch(iscrizioneForm.action, {
      method: "POST",
      body: formData,
    ...
    if (response.ok) {
      ...
      throw new Error("Errore durante l'invio dell'iscrizione (Formspree).");
    }
